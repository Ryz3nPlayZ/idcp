...
fn main() {
    let workloads = [
        exact_duplicate_workload(),
        near_duplicate_workload(),
        mixed_workload(),
    ];

    println!(
        "{:<20} {:>8} {:>8} {:>8} {:>10} {:>10} {:>10}",
        "workload", "pages", "exact", "near", "raw_mib", "smart_mib", "savings"
    );

    for workload in workloads {
        let report = analyze_workload(&workload);
        println!(
            "{:<20} {:>8} {:>8} {:>8} {:>10.2} {:>10.2} {:>9.1}%",
            workload.name,
            report.page_count,
            report.exact_duplicate_pages,
            report.near_duplicate_pages,
            mib(report.raw_bytes),
            mib(report.estimated_bytes),
            report.savings_percent(),
        );
        println!(
            "  families={} unique={} exact_saved_mib={:.2} delta_saved_mib={:.2} best_delta={:.1}% avg_chunk_similarity={:.1}%",
            report.family_count,
            report.unique_pages,
            mib(report.exact_saved_bytes),
            mib(report.delta_saved_bytes),
            report.best_delta_savings_percent,
            report.avg_chunk_similarity_percent,
        );
    }
}
...
struct AnalysisReport {
    ...
    unique_pages: usize,
    family_count: usize,
    exact_duplicate_pages: usize,
    near_duplicate_pages: usize,
    raw_bytes: usize,
    estimated_bytes: usize,
    exact_saved_bytes: usize,
    delta_saved_bytes: usize,
    best_delta_savings_percent: f64,
    avg_chunk_similarity_percent: f64,
}
...
fn analyze_workload(workload: &Workload) -> AnalysisReport {
    ...
    for bucket in buckets.values() {
        ...
        for &candidate in &bucket[1..] {
            if exact_groups[&infos[candidate].hash].len() > 1 {
                continue;
            }
...
fn delta_encoded_size(base: &[u8; PAGE_SIZE], other: &[u8; PAGE_SIZE]) -> usize {
    ...
    while idx < PAGE_SIZE {
        ...
        encoded += DELTA_HEADER_BYTES + run_len;
    }
    encoded
}
...
fn exact_duplicate_workload() -> Workload {
...
fn near_duplicate_workload() -> Workload {
    let mut pages = Vec::new();
